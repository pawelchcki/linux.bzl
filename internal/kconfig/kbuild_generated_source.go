...
//
// A handful of leaf objects have no source file in the tree at all. Their
// source is produced by a per-directory Kbuild rule, for example
//
//	$(obj)/%.S: $(src)/%.pl FORCE
//		$(call if_changed,perlasm)
//
// Historically these were matched by hardcoded object-path tables, which
// encode one kernel version's directory layout and silently misdescribe
// another. This file derives the same information from the rules the parser
// already produces, so a new kernel's new rule is either understood or
// reported with the exact Makefile line that needs attention.
//
// Interpretation lives beside the parser because only the parser knows
// rootDir: rule prerequisites arrive "$(src)"-expanded and therefore absolute,
// and turning them back into source-relative paths needs that root. Nothing
// here consults the compact metadata options, so the fact that opts.SourceRoot
// happens to hold the same value cannot be relied upon by accident.
//
// Two deliberate divergences from GNU make, both bounded by what Kbuild
// actually writes:
//
//   - No directory-split implicit matching. GNU make will try to match a
//     pattern without a slash against the file part of a target in any
//     directory. Every rule that matters here is "$(obj)/"-prefixed, so its
//     pattern always contains a slash and a plain whole-path match is
//     equivalent.
//   - One level of rule chaining, not a recursive implicit search. A generated
//     source is produced directly from checked-in files; nothing in the
//     supported set generates an input that is itself generated by another
//     per-directory rule.

// kbuildPhonyPrerequisites are the prerequisite names that carry no file.
//
// Kbuild's ".PHONY" declarations are not tracked by the parser, so this list
// is deliberately closed: an unrecognised bare word is reported rather than
// guessed at. That is what surfaces a new kernel's new phony target instead of
// silently dropping a real input from the digest.
...
type KbuildGeneratedSource struct {
	...
	CommandName string
	// Explicit records whether an explicit rule matched, as opposed to a
	// pattern rule.
	Explicit bool
	...
	for _, rule := range candidates {
		...
		for _, candidate := range rule.Targets {
			if filepath.ToSlash(candidate) == target {
				explicit = ruleMatch{rule: rule, explicit: true}
				found = true
				break
			}
		}
...
func (r *KbuildGeneratedSourceResolver) buildGeneratedSource(object, target string, match ruleMatch) (KbuildGeneratedSource, error) {
	...
	orderOnly, err := resolve(rule.OrderOnly, "order-only prerequisite")
	if err != nil {
		return KbuildGeneratedSource{}, err
	}
	...
	for _, prerequisite := range orderOnly {
		...
		generated.OrderOnly = append(generated.OrderOnly, prerequisite)
	}
